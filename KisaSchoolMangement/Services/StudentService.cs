                ...
                using (var conn = new MySqlConnection(_connectionString))
                {
                    ...
                    string query = @"INSERT INTO students 
                            (gr_no, student_name, dob, gender, class_id, section_id, 
                             ...
                             photo, cnic_image, father_cnic_photo, mother_cnic_photo) 
                            VALUES 
                            (@GrNo, @StudentName, @DOB, @Gender, @ClassId, @SectionId, 
                             ...
                             @Photo, @CNICImage, @FatherCNICPhoto, @MotherCNICPhoto)";
                    ...
                    using (var cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        // Photo Paths
                        cmd.Parameters.AddWithValue("@Photo", string.IsNullOrEmpty(student.Photo) ? DBNull.Value : (object)student.Photo);
                        cmd.Parameters.AddWithValue("@CNICImage", string.IsNullOrEmpty(student.CNICImage) ? DBNull.Value : (object)student.CNICImage);
                        cmd.Parameters.AddWithValue("@FatherCNICPhoto", string.IsNullOrEmpty(student.FatherCNICPhoto) ? DBNull.Value : (object)student.FatherCNICPhoto);
                        cmd.Parameters.AddWithValue("@MotherCNICPhoto", string.IsNullOrEmpty(student.MotherCNICPhoto) ? DBNull.Value : (object)student.MotherCNICPhoto);
            ...
            try
            {
                using (var conn = new MySqlConnection(_connectionString))
                {
                    ...
                    SET gr_no = @GrNo,
                        ...
                        father_cnic_photo = @FatherCNICPhoto, 
                        mother_cnic_photo = @MotherCNICPhoto
                    WHERE id = @Id";
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                return new StudentModel
                                {
                                    ...
                                    GrNo = reader["gr_no"] != DBNull.Value ? reader["gr_no"].ToString() : "",
                                    StudentName = reader["student_name"] != DBNull.Value ? reader["student_name"].ToString() : "",
                                    // ... populate all other fields similar to GetAllStudents ...
                                    // Mapping same as GetAllStudents() - duplicating full mapping here for completeness in real app
                                    // For brevity, assuming user uses GetAllStudents mostly or this is synced logic.
