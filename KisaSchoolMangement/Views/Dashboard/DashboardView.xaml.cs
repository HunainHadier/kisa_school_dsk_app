...
namespace KisaSchoolMangement.Views.Dashboard
{
    ...
    {
        // _currentUser may be null for design-time ctor; mark nullable
        private readonly AuthService.User? _currentUser;

        // initialize timer so non-nullable warnings go away
        private readonly DispatcherTimer _timer = new DispatcherTimer();

        // Constructor without parameters (Design time)
        ...
        // Replaced placeholder message boxes with real windows so Roles / Permissions open properly.
        private void ManageRoles_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Optionally focus Roles list
                win.ShowDialog();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error opening Roles window: {ex.Message}", "Error",
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void ManagePermissions_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                win.Owner = this;
                win.WindowStartupLocation = WindowStartupLocation.CenterOwner;
                // Optionally focus Permissions section inside the same window
                win.ShowDialog();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error opening Permissions window: {ex.Message}", "Error",
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
